...
/* -------------------------------------------------------------------------
   The same job, from the barangay's side of the desk.

   An official's home screen asks a different question — not "what is
   happening to me" but "what is this barangay behind on". The old one
   answered neither: its headline number was the TOTAL incidents, with the
   count that actually needed acting on demoted to a caption underneath.
------------------------------------------------------------------------- */

/** Reports nobody has been sent to yet — the queue that matters most. */
export function unassignedIncidents(incidents = []) {
  return incidents.filter(i => !isSettled(i?.status) && !i?.assigned_to)
}

export function officialSummary({
  incidents = [], tickets = [], documentsOpen = 0, documentsBreached = 0,
  casesOpen = 0, casesBreached = 0, verificationsPending = 0,
  tanodsOnDuty = 0, tanodsTotal = 0, barangayName,
} = {}) {
  const where = barangayName ? `In ${barangayName}` : 'Here'
  ...
  if (unassigned > 0) {
    parts.push(`${plural(unassigned, 'report has', 'reports have')} nobody assigned`)
  } else if (openIncidents > 0) {
    parts.push(`${plural(openIncidents, 'report is', 'reports are')} being worked`)
  }
  if (documentsBreached > 0) {
    parts.push(`${plural(documentsBreached, 'document request is', 'document requests are')} past the RA 11032 deadline`)
  } else if (documentsOpen > 0) {
    parts.push(`${plural(documentsOpen, 'document request is', 'document requests are')} waiting`)
  }
  if (casesBreached > 0) {
    parts.push(`${plural(casesBreached, 'blotter case is', 'blotter cases are')} past a Katarungang Pambarangay deadline`)
  } else if (casesOpen > 0) {
    parts.push(`${plural(casesOpen, 'blotter case is', 'blotter cases are')} before the Lupon`)
  }
  if (openTickets > 0) {
    parts.push(`${plural(openTickets, 'ticket needs', 'tickets need')} a reply`)
  }
  if (verificationsPending > 0) {
    parts.push(`${plural(verificationsPending, 'resident is', 'residents are')} waiting to be verified`)
  }

  // Nobody on duty is worth saying even on a quiet day: it is the reason
  // the next report will sit unassigned.
  const dutyNote = tanodsTotal > 0 && tanodsOnDuty === 0
    ? ' No tanod is on duty right now.'
    : ''
  ...
  if (parts.length === 0) {
    return {
      allClear: dutyNote === '',
      text: (barangayName
        ? `Nothing is outstanding in ${barangayName}.`
        : 'Nothing is outstanding.') + dutyNote,
    }
  }

  return { allClear: false, text: `${where}: ${joinClauses(parts)}.${dutyNote}` }
}

/** The one thing an official should look at before anything else. */
export function officialMostPressing({ incidents = [] } = {}) {
  const pool = unassignedIncidents(incidents)
  const critical = pool
    .filter(i => i.priority === 'Critical')
    .sort((a, b) => new Date(a.created_at) - new Date(b.created_at))[0]
  const oldest = pool.sort((a, b) => new Date(a.created_at) - new Date(b.created_at))[0]
  const pick = critical || oldest
  if (!pick) return null

  return {
    kind: 'incident',
    tone: pick.priority === 'Critical' ? 'urgent' : 'watch',
    id: pick.id,
    title: pick.title,
    detail: pick.priority === 'Critical'
      ? 'Critical, and no tanod has been dispatched'
      : 'Waiting for a tanod to be dispatched',
    href: `/official/incident/${pick.id}`,
  }
}
