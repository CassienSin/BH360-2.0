...
export async function consumeAiQuota(userId, route) {
  ...
  try {
    ...
    // A missing env var and a database blip both land here, and they need
    // different words. "Temporarily unavailable" invites waiting, which is
    // exactly wrong when the fix is a deploy setting — this route is the
    // only runtime consumer of the service-role key, so forgetting it in
    // Vercel takes the AI features down and nothing else, with no clue why.
    const misconfigured = /SUPABASE_SERVICE_ROLE_KEY|Missing environment variable/i
      .test(err?.message || '')
    if (misconfigured) {
      console.error(
        `[config] ${route} is disabled: SUPABASE_SERVICE_ROLE_KEY is not set on this ` +
        'deployment. It is required for AI rate limiting. Add it in Vercel → Settings → ' +
        'Environment Variables (server-side, no NEXT_PUBLIC_ prefix) and redeploy.'
      )
    } else {
      console.error(`Rate limit check failed for ${route}:`, err)
    }

    // Fails closed either way: the point of this check is protecting a
    // spend limit, and an unlimited AI endpoint is the thing it exists to
    // prevent. The message above is what makes that recoverable.
    return {
      allowed: false,
      used: 0,
      limit: config.limit,
      ...
      message: misconfigured
        ? 'The AI assistant is not configured on this deployment. Please tell your barangay administrator.'
