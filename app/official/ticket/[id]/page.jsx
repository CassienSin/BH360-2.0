    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-brand">
      ...
      <header className="bg-white relative z-10 px-6 py-4 flex items-center gap-3"
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">From: {ticket?.profiles?.full_name}</p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {ticket && (
            <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${statusColor[ticket.status]}`}>
          ...
          {messages.map(msg => (
            <div key={msg.id} className={`flex ${msg.sender_id === user?.id ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl text-sm shadow-sm ${
                msg.sender_id === user?.id ? 'text-white' : 'bg-white text-gray-800'
              }`}
